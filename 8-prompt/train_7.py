# 思路：prompt+lstm
# 把最优的label_words挑出来，然后送入lstm里

# 最简单的prompt，后续工作都以本代码为基础进行魔改
...
from util import build_embedding_matrix
...
class Trainer:
    ...
    def Define_PromptModel(self):
        ...
        self.rnn_model = LSTM(self.embedding_matrix, self.opt).to(self.opt.device)
    ...
    def train(self):
        ...
        for epoch in range(self.opt.max_epoch):
            ...
            for step, inputs in enumerate(self.train_dataloader):
                ...
                # outputs_at_mask 在mask处的30522维的向量，对应每一个单词的预测值
                outputs_at_mask = self.prompt_model.forward_without_verbalize(inputs)
                # label_words_logits 在outputs_at_mask中把label_words对应的分数提取出来
                label_words_logits = outputs_at_mask[:, self.all_label_words_ids]

                logits_1 = self.prompt_model.verbalizer.process_outputs(outputs_at_mask, inputs)
                loss_1 = loss_func(logits_1, labels)             

                # 分数最高的label_words
                batch_best_label_words = []
                # 找出每一条训练数据,在verbalizer中排名前n的单词
                for each_label_words_score in label_words_logits:
                    best_label_words_indexs = each_label_words_score.argsort(descending=True).tolist()[:5]
                    best_label_words = [self.all_label_words_list[index] for index in best_label_words_indexs]
                    best_label_words = self.tokenizer.convert_tokens_to_ids(best_label_words)
                    batch_best_label_words.append(best_label_words)

                batch_best_label_words = torch.tensor(batch_best_label_words).to(self.opt.device)
                logits_2 = self.rnn_model([batch_best_label_words])
